...
//            $this->pac_content = preg_replace($regx_i, $args[$i],
 //$this->pac_content);
//        }
//    }
...
class pac {    
    ...
    public $socks5_servers = array();
    public $socks_servers = array();
    public $http_servers = array();
    public $https_servers = array();
    // </editor-fold>
    
    // <editor-fold defaultstate="collapsed" desc="rule set">
    public $proxy_host = array();
    public $proxy_ip = array();
    public $direct_host = array();
    public $direct_ip = array();
    public $adbp_filters = array();
    ...
    public function get_pac() {
        ...
        foreach ($this->socks_servers as $server) {
            $proxy .= "SOCKS {$server}; ";
        }
        foreach ($this->http_servers as $server) {
            $proxy .= "PROXY {$server}; ";
        }
        foreach ($this->https_servers as $server) {
            $proxy .= "HTTPS {$server}; ";
        }
        $proxy .= "DIRECT";
        $proxy_host = json_encode($this->hostname_filter($this->proxy_host));
        $proxy_ip = json_encode($this->proxy_ip);
        $direct_host = json_encode($this->hostname_filter($this->direct_host));
        $direct_ip = json_encode($this->direct_ip);
        $adbp_filters = json_encode($this->adbp_filters);
...
EOF;
        ...
        $script .= <<<'EOF'
            ...
            for (var i = 0; i < adbp_filters.length; i++) {
              defaultMatcher.add(Filter.fromText(adbp_filters[i]));
            }
EOF;
        // </editor-fold>
        $script .= <<<'EOF'
            ...
            function test_ipv4_address(addr, list) {
                for (var i in list) {
                    if (isInNet(addr, list[i][0], list[i][1])) {
                        return true;
                    }
                }
                return false;
            ...
            function test_host_name(host, list) {
                for (var i in list) {
                    if (is_suffix(host, list[i])) {
                        return true;
                    }
                }
                return false;
            ...
            function FindProxyForURL(url, host){
                if (default_proxy == "proxy") {
                    // test direct_host and direct_ip only
                    
                    var ip = host;
                    if (!is_ipv4_address(ip)) {
                        ip = dnsResolve(host);
                    }
                    if (test_ipv4_address(ip, direct_ip)) {
                        return direct;
                    }
                    if (test_host_name(host, direct_host)) {
                        return direct;
                    }
                ...
                } else if (default_proxy == "direct") {
                    // test proxy_host and proxy_ip only
                    
                    var ip = host;
                    if (!is_ipv4_address(ip)) {
                        ip = dnsResolve(host);
                    }
                    if (test_ipv4_address(ip, proxy_ip)) {
                        return proxy;
                    }
                    if (test_host_name(host, proxy_host)) {
                        return proxy;
                    }
